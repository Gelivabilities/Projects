function acc=Validation(data,classes,num_of_epoch,mode,algorithm,parameter)
    %initialize variables
    rows=size(data,1);
    cols=size(data,2);
    acc=zeros(num_of_epoch,6);
    ...
    for i=1:num_of_epoch
        ...
        switch mode
            case 'LOO'
                acc(i,:)=[Get_Accuracy(train_dataset,W),Get_Accuracy_for_LOO(test_dataset,shuffled_classes(i,:),W)];
            case 'cross'
                acc(i,:)=[Get_Accuracy(train_dataset,W),Get_Accuracy(test_dataset,W)];
                fprintf('%d: train: %.4f %.4f %.4f, test: %.4f %.4f %.4f\n',i,acc(i,1),acc(i,2),acc(i,3),acc(i,4),acc(i,5),acc(i,6));
        end
